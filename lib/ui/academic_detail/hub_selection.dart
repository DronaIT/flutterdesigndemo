...
class _HubSelectionState extends State<HubSelection> {
  ...
  Future<void> initialization() async {
    ...
    try{
      ...
      if (data.records?.isNotEmpty == true) {
        PreferenceUtils.setHubList(data);
        setState(() {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        child: Scaffold(
      appBar: AppWidgets.appBarWithoutBack(strings_name.str_hubs),
      body: Stack(children: [
        hubData?.isNotEmpty == true
            ? Column(children: [
                ...
                CustomButton(
                  ...
                  click: () {
                    List<BaseApiResponseWithSerializable<HubResponse>>? selectedHubData = [];
                    for (var i = 0; i < hubData!.length; i++) {
                      if (hubData![i].fields!.selected) {
                        selectedHubData.add(hubData![i]);
                      }
                    }
                    if (selectedHubData.isNotEmpty) {
                      Get.back(result: selectedHubData);
                    } else {
                      Utils.showSnackBar(context, strings_name.str_select_hub);
                    }
                  },
                )
              ])
